<template>
  ...
  <v-row style="margin-left: 180px;" class="d-none d-sm-flex">
    <v-window v-model="type">
      ...
      <v-row class="main-b2b" v-for="(orders, id) in ordersall">
        ...
        <v-row v-else>
          ...
          <v-col style="min-width: 250px;">
            <v-row class="letter-white">
              {{ orders.company }} , {{ orders.order_id }}
            </v-row>  
            <v-row class="letter-blue">
              {{ orders.companyOrdersCount }} исполнено | {{ orders.company_orders_count_complete_percent * 100 }} %
            </v-row>  
            <v-row class="letter-blue">
              В сети
            </v-row>  
  ...
  <v-window-item :value="3">
    ...
    <v-row class="main-b2b" v-for="(orders, id) in ordersall">
          ...
          <v-col style="min-width: 130px;">
            Курс <div class="letter-white2 dopvcol">{{orders.course}} </div>
          </v-col>
          <v-col style="min-width: 120px;">
            <div class="letter-rub dopvcol2">{{ orders.currency }}</div>
          </v-col>
          ...
          <v-col style="min-width: 250px;">
            ...
            <v-btn color="success" @click="confirmCheckpay(orders)">
                   check-pay
                  </v-btn>
    ...
    <v-row v-if="showdetails[id]">
        ...
        <v-col>
          ...
          <v-row>
            ...
            <v-btn color="info" @click="confirmEdit(id)">
              edit=order
            </v-btn>
  ...
  <v-dialog v-model="dialog2" max-width="400px">
          ...
          <v-row>
            ...
            <v-col class="text-right">
              <v-card-item>
                ...
                <div class="ms-2">
                  <h5><b>{{ selectedOrder.companyOrdersCount }}</b> ордер(а,ов) </h5>
                  <h5><b>{{ selectedOrder.company_orders_count_complete_percent * 100}} %</b> выполнено</h5>
                </div>
              </v-card-item>
            </v-col>
      ...
      <v-card >
        ...
        <v-combobox v-model="createOkved" :items="okveds" label="Окведы" item-title="title" item-value="id"
    ...
    mounted() {
      ...
      //получаем список ОКВЕД
      api.get('api/company/get-okveds', {
          headers: {
          ...
          then(response => {
            ...
            this.okveds = response.data
            //console.log(this.charts)
            //localStorage.setItem('b2b_data', JSON.stringify(response.data))
    ...
    data: () => ({
      ...
      selectedRefresh: 0,
      durations: [
        { id: 0, name: "3 рабочих дня" },
      ],
      // myDurations: [
      //   { id: 0, name: "3 дня", timing: 60*60*24*3 },
      // ],
      myOkved: [
      { id: 0, title: "61.54 - Добыча угля"},
      ],
      selectedDurations: 0,
      secondscount: 59,
      minutescount: 15,
    ...
    computed: {
        sellerPayment(){
          this.loader = true
          //console.log(this.companypayment)
          //console.log(this.selectedPayment)
          
          //console.log(this.companypayment.find(so => so.payment_id === this.selectedPayment))
          if (this.selectedPayment !== null) {
            if (this.companypayment.find(so => so.payment_id === this.selectedPayment)) {
        ...
        getOkveds() {
          api.get('api/company/get-okveds' + this.selectedCurrency, {
              headers: {
                'Authorization': localStorage.getItem('access_token')
              }
            }).
      ...
      methods: {
        ...
        getFulllist() {
          ...
          if (this.type == 3) {
            ...
            str = 'history?status_history=1,2' //в обработке
          }
          if (this.type == 4) { //мои ордера
    
            //console.log(profile)
            str = 'full-list?status=-1,6&company_id=' + profile.id
          }
          if (localStorage.getItem('payments')=="undefined" || this.selectedPayments == null) {
            str += ''
          } else {str += '&payments=' + this.selectedPayments }
          
          if (this.type == 5) { str='history?status_history=4,6,7,9&company_id='+profile.id } // история
        ...
        cancelOrder(id) {
          ...
          if (this.selectedOrder.order_id == null) {
            ...
            var send = {
              b2b_ads_id: this.selectedOrder.order_id,
            }
    
            api({
              method: 'post',
              url: '/api/b2b/cancel-order',
              headers: {
                'Authorization': localStorage.getItem('access_token')
              },
              data: send
        ...
        confirmCreate(){
          ...
          if (this.createAmount == null || this.createChart_id == null || this.createCourse == null || 
          this.createmax_limit == null || this.createmin_limit == null) {
            alert("Введены не все данные")
          // this.dialogCreate = false
          } else {
            
            console.log(this.createOkved.id)
            var send = {
              ...
              main_okved: this.createOkved.id,
              duration: 3,
        ...
        confirmCheckpay(id) {
          ...
          if (id.b2b_ads_id == null) {
            alert("Не верные данные")
            this.dialogCheckpay = false
          } else {
    
            //let formData = new FormData();
            // formData.append('p2p_ads_id', this.selectedOrder.order_id)
            // formData.append('imageFile', this.image[0])
            // console.log('>> formData >> ', formData)
            let send = {
               //image: this.image[0],
               b2b_ads_id: id.b2b_ads_id
            }
            //console.log('>> formData >> ', formData)
            api({
              method: 'post',
              url: '/api/b2b/check-pay',
              headers: {
                'Authorization': localStorage.getItem('access_token'),
        ...
        confirmPayment(id){
          ...
          this.selectedOrder = this.ordersall[id]
          //console.log(this.selectedOrder)
          console.log(this.selectedOrder.b2b_ads_id)
          if (this.selectedOrder.b2b_ads_id == null) {
            alert("Не верные данные")
            
          } else {
    
            var send = {
              b2b_ads_id: this.selectedOrder.b2b_ads_id,
              
            }
    
            api({
              method: 'post',
              url: '/api/b2b/confirm-payment',
              headers: {
                'Authorization': localStorage.getItem('access_token')
              },
              data: send
        ...
        cancelTrade(id){
          console.log(this.ordersall[id])
          var send = {
              b2b_ads_id: this.ordersall[id].b2b_ads_id,
              
            }
    
            api({
              method: 'post',
              url: '/api/b2b/cancel-trade',
              headers: {
                'Authorization': localStorage.getItem('access_token')
              },
              data: send
