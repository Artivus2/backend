...
class B2bController extends BaseController
{
    ...
    public function actionConfirmTrade()
    {
        ...
        if ($b2b_ads->type == 1) {
            ...
            // chat_room b2b
            $chat_id = $this->newChatroom($b2b_h->start_date, $b2b_h->author_id, $b2b_h->creator_id);
        ...
        foreach ($b2bAds_query as $item)
            {   
                ...
                if ($item->ads->company_id == $this->user->id) {
                    ...
                    "author_bank" => $author_payments->bank,
                    "author_bik" => $author_payments->bik,
                    "author_rs" => $author_payments->value,
                    "author_ks" => $author_payments->ks,
                    "author_phone" => $author_info->phone ?? null,
                    "avatar_author" => $item->user->avatar,
                    "creator" => $item->company->name,
                    "creator_id" => $item->creator_id,
                    "creator_bank" => $creator_payments->bank,
                    "creator_bik" => $creator_payments->bik,
                    "creator_rs" => $creator_payments->value,
                    "creator_ks" => $creator_payments->ks,
